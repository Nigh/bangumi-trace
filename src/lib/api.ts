...
async function request(path: string, init?: RequestInit) {
  ...
  if (!response.ok) throw Object.assign(new Error((await response.json().catch(() => null))?.error || response.statusText), { status: response.status })
  return response
}
...
export async function searchBangumi(query: string) {
  return (await request(`/api/bangumi/search?q=${encodeURIComponent(query)}`)).json() as Promise<{ data: { id: number; name: string; name_cn?: string; images?: { small?: string }; date?: string }[] }>
}
