<script lang="ts">
  ...
  async function sync(force = true, confirmed = false) {
    ...
    busy = true
    try { const loaded = await loadData(); data = loaded.data; sha = loaded.sha; auth = "authenticated"; cache(loaded.migrated); if (view === "list" || view === "folder") refreshListOrder(); if (loaded.migrated) message("数据已升级，请保存到 GitHub"); else if (force) message("已用 GitHub 数据覆盖本地内容") }
    catch (error) {
