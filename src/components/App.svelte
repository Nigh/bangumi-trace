<script lang="ts">
  ...
  let importBackup: BangumiData | null = null, repositoryDialog: HTMLDialogElement, confirmDialog: HTMLDialogElement, addShowDialog: HTMLDialogElement, addVolumeDialog: HTMLDialogElement, folderDialog: HTMLDialogElement, moveDialog: HTMLDialogElement
  ...
  let newTitle = "", addToFolderId = "", folderName = "", editingFolderId = "", metadataQuery = "", candidates: Awaited<ReturnType<typeof searchBangumi>>["data"] = [], metadataBusy = false, metadataSearched = false, metadataError = ""
  ...
  onMount(() => {
    ...
    if (cached) try { const value = JSON.parse(cached); const normalized = normalizeBangumiData(value.data); if (!normalized) throw new Error(); data = normalized; sha = value.sha; dirty = value.data?.version === 3; refreshListOrder() } catch { localStorage.removeItem(CACHE) }
    void sync(false)
  ...
  async function sync(force = true, confirmed = false) {
    ...
    busy = true
    try { const loaded = await loadData(); data = loaded.data; sha = loaded.sha; volumeUndo = new Map(); auth = "authenticated"; cache(loaded.migrated); if (view === "list" || view === "folder") refreshListOrder(); if (loaded.migrated) message("数据已升级，请保存到 GitHub"); else if (force) message("已用 GitHub 数据覆盖本地内容") }
    catch (error) {
...
{#if auth !== "authenticated"}
  <main class="grid min-h-dvh place-items-center p-4 text-center">
    <div class="w-full max-w-sm rounded-box border border-base-300 bg-base-100 p-8 shadow-sm">
      ...
      {#if auth === "checking"}<p class="mt-5" aria-busy="true">正在检查登录状态…</p>{:else if auth === "repository-error"}<h1 class="mt-5 text-xl font-bold">数据仓库不可用</h1><p class="mt-2 text-base-content/70">完成仓库配置后即可继续使用。</p><button class="btn btn-primary mt-5 w-full" disabled={busy} on:click={() => sync(false)}>重试</button>{:else}<h1 class="mt-5 text-2xl font-bold">Bangumi Trace</h1><p class="mt-2 text-base-content/70">使用 GitHub 同步你的私人观看数据。</p><a class="btn btn-primary mt-5 w-full" href={loginUrl}>使用 GitHub 登录</a>{/if}
    </div>
  </main>
{:else}
  ...
  <main class="mx-auto max-w-3xl p-4 pb-24">
    ...
    {#if view === "list" || view === "folder" && selectedFolder}
      <section class="space-y-4">
        {#if view === "folder"}<button class="btn btn-ghost btn-sm" on:click={() => go("list")}>← 返回我的番剧</button>{/if}
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          {#each visibleFolders as folder}<button class="card border border-base-300 bg-base-200/50 text-left transition hover:border-primary focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary" on:click={() => go(`folder/${folder.id}`)}><span class="card-body p-4"><span class="flex items-start justify-between gap-3"><strong class="text-lg">{folder.name}</strong><span class="badge badge-outline whitespace-nowrap">{folder.showIds.length} 部</span></span><span class="text-sm text-base-content/60">系列目录 · 点击查看作品</span></span></button>{/each}
          {#each shows as show}{@const next = nextVolumeEpisode(data, show)}{@const status = statusDisplay(data, show)}{@const owner = data.folders.find((folder) => folder.showIds.includes(show.id))}<article class="card border border-base-300 bg-base-100"><div class="card-body p-4"><div class="flex items-start justify-between gap-2"><button class="min-w-0 break-words text-left text-lg font-semibold hover:text-primary" on:click={() => go(`show/${show.id}`)}>{show.title[0]}</button><span class={`badge shrink-0 whitespace-nowrap ${status.badge}`}>{status.label}</span></div>{#if owner && view === "list" && query.trim()}<p class="text-sm text-base-content/60">目录：{owner.name}</p>{:else if show.title.length > 1}<p class="truncate text-sm text-base-content/60">{show.title.slice(1).join(" · ")}</p>{/if}<div class="card-actions mt-auto justify-end">{#if view === "folder"}<button class="btn btn-ghost btn-sm" on:click={() => { data = { ...data, folders: data.folders.map((folder) => folder.id === selectedFolder!.id ? { ...folder, showIds: folder.showIds.filter((id) => id !== show.id) } : folder) }; cache() }}>移出目录</button>{/if}{#if next}<button class="btn btn-primary btn-sm" aria-label={`标记已观看${volumeLabel(show, next.volume)}第 ${next.episode} 话`} on:click={() => quickRecord(show, next.volume)}>记录 {volumeLabel(show, next.volume)}第 {next.episode} 话</button>{:else}<button class="btn btn-sm" disabled>{show.volumes.length ? "已全部看完" : "请先添加 Volume"}</button>{/if}</div></div></article>{/each}
        </div>
    ...
    {:else if view === "show" && selected}
      <section class="space-y-5"><button class="btn btn-ghost btn-sm" on:click={() => go(selectedOwner ? `folder/${selectedOwner.id}` : "list")}>← 返回</button><div class="flex flex-col items-start justify-between gap-3 sm:flex-row"><div class="min-w-0"><h1 class="text-3xl font-bold">{selected.title[0]}</h1><div class="flex flex-wrap items-center gap-x-2 text-sm text-base-content/60">{#each selected.title.slice(1) as title, index}{#if index}<span aria-hidden="true">·</span>{/if}<span class="relative inline-flex"><button class="hover:text-primary" aria-expanded={activeSubtitle === index + 1} on:click|stopPropagation={() => activeSubtitle = activeSubtitle === index + 1 ? -1 : index + 1}>{title}</button>{#if activeSubtitle === index + 1}<span class="absolute left-1/2 top-full z-20 mt-2 -translate-x-1/2 rounded-box border border-base-300 bg-base-100 p-1 shadow-lg" on:click|stopPropagation><button class="btn btn-primary btn-xs whitespace-nowrap" on:click={() => { updateShow(setDefaultTitle(selected!, index + 1)); activeSubtitle = -1 }}>设为默认</button></span>{/if}</span>{/each}</div><p class="text-base-content/60">{selected.externalRef ? `Bangumi #${selected.externalRef.id}` : "本地作品"}</p></div><div class="w-full sm:w-auto"><fieldset class="grid grid-cols-2 gap-1 rounded-box bg-base-200 p-1 sm:grid-cols-4" aria-label="观看状态">{#each statuses as item}<button class:border-primary={selected.status === item.value} class:bg-base-100={selected.status === item.value} class="btn btn-ghost btn-sm whitespace-nowrap border border-transparent" type="button" aria-pressed={selected.status === item.value} on:click={() => updateShow({ ...selected!, status: item.value })}>{item.label}</button>{/each}</fieldset><p class="mt-2 max-w-md text-xs text-base-content/60">{selected.status === "watching" && selectedComplete ? "已看完当前全部剧集，保持追番中可能是在等待更新。" : selected.status === "completed" && !selectedComplete ? "尚未看完全部剧集，但已手动标记为完成。" : `当前状态：${statusDisplay(data, selected).label}`}</p></div></div>
