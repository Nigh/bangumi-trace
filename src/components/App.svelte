<script lang="ts">
  ...
  import { emptyData, episodeLabel, eventTime, expandedEpisodes, matchesTitle, nextEpisode, nextVolumeEpisode, reorderTitle, sortShowsByActivity, uniqueTitles, volumeLabel, type BangumiData, type Precision, type Show, type Status, type Volume, type WatchEvent } from "../lib/model"
  ...
  let volumeId = "", episodeFrom = 1, episodeTo = 1, precision: Precision = "exact", watchedValue = "", rangeFrom = "", rangeTo = "", rangeLabel = "近期", editingEventId = ""
  ...
  function addShow() {
    ...
    const show: Show = { id: crypto.randomUUID(), title: [newTitle.trim()], status: "planned", volumes: [], notes: [] }
  ...
  function startRecord(show: Show, event?: WatchEvent) {
    ...
    precision = event?.watchedAt.precision ?? "exact"
    watchedValue = event?.watchedAt.precision === "exact" ? event.watchedAt.value.slice(0, 16) : event?.watchedAt.precision === "day" ? event.watchedAt.value : nowForInput()
